...
import WorldMapSvg from './assets/world.svg'; // SVG map as component
...
const currentVpn = vpnList.find((v) => v.file === selectedVpn);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Offline World Map Highlight */}
      {connected && currentVpn && (
  ...
  if (countryName && countryName === currentVpn.name.toLowerCase()) {
    return React.cloneElement(child, { fill: '#e53935' });
  }

  return React.cloneElement(child, { fill: '#ccc' });
})}
          </Svg>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  powerIconWrapper: { position: "absolute", top: 35, left: 35, width: 52, height: 52, borderRadius: 26, alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 5 },
